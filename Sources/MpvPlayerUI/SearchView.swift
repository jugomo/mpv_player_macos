...
/// (ver `SearchViewModel`/`YtDlpSearchFetcher`). Tocar un resultado lo
/// reproduce a través de `PlayerViewModel.play(urlString:)`, el mismo
/// camino que reproducir una URL pegada a mano.
struct SearchView: View {
        ...
        if !playerViewModel.status.isYtdlpInstalled {
            ...
            List(viewModel.results) { result in
                resultRow(result)
                    ...
                    .onTapGesture { play(result) }
                    .help(loc.t(.playSearchResultTooltip))
            }
            .listStyle(.inset)
    ...
    private func play(_ result: SearchResult) {
        playerViewModel.play(urlString: result.watchURLString)
